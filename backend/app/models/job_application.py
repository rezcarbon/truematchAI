...
class ApplicationTimeline(Base, TimestampMixin):
    """
    Timeline event for a job application.

    Maintains chronological record of all events throughout an application's
    lifecycle, enabling audit trail and pipeline analytics.

    Attributes:
        id: Unique identifier for this timeline event
        application_id: Foreign key to JobApplication
        event_type: Type of event that occurred
        event_data: Structured data about the event (encrypted)
    ...
    event_data: Mapped[Optional[dict]] = mapped_column(EncryptedJSON, nullable=True)
    ...
    __table_args__ = (
        ...
        Index("ix_application_timelines_event_type", "event_type"),
        Index("ix_application_timelines_created_at", "created_at"),
        Index("ix_application_timelines_app_type", "application_id", "event_type"),
        Index("ix_application_timelines_app_created", "application_id", "created_at"),
    )

    def __repr__(self) -> str:
        """Concise repr for debugging."""
        return (
            f"<ApplicationTimeline id={self.id!r} application_id={self.application_id!r} "
    ...
    def to_dict(self) -> dict:
        ...
        return {
            ...
            "event_type": self.event_type.value if isinstance(self.event_type, ApplicationEventType) else self.event_type,
