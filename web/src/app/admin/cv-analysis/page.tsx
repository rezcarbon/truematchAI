...
export default function AdminCVAnalysisPage() {
  ...
  useEffect(() => {
    const fetchResumes = async () => {
      try {
        ...
        const response = await fetch(`${backendUrl}/api/v1/files/resumes`);
        if (!response.ok) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!form.targetSeniority) {
      setError('Please select the target seniority level');
      return;
    }
    ...
    try {
      // Use backend API directly (port 8000)
      const backendUrl = `${window.location.protocol}//${window.location.hostname}:8000`;
      const response = await fetch(`${backendUrl}/api/v1/candidates/cv-analysis`, {
        method: 'POST',
        headers: {
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title="Analyze Candidate CV"
        subtitle="Analyze candidate CVs to identify skill gaps and career opportunities"
        icon="Sparkles"
      />
      ...
      <div className="container mx-auto px-4 py-8 max-w-2xl">
        <Card>
          <CardHeader>
            <CardTitle>Candidate CV Analysis</CardTitle>
            <p className="mt-2 text-sm text-muted-foreground">
              Analyze a candidate's CV against target roles to identify skill gaps, career opportunities, and suggest improvements.
            </p>
          </CardHeader>
          <CardContent>
            <form onSubmit={handleSubmit} className="space-y-6">
                ...
                {loadingResumes ? (
                  ...
                  <div className="rounded-lg border border-dashed p-4 text-center">
                    <p className="text-sm text-muted-foreground">
                      No resumes found. Candidates must upload resumes before analysis.
                    </p>
                  </div>
                ) : (
                  <select
              ...
              <div className="space-y-2">
                ...
                <p className="text-xs text-muted-foreground">
                  The target role to analyze against
                </p>
              </div>
